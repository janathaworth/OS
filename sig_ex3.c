...
int main(void) {
  ...
  if (pid_ch1 > 0){
	...
	if (WIFEXITED(status)) {
      ...
      printf("%d\n", getpgid(0));
      // fprintf(stderr, "in 2\n");
      int err = setpgid(0,pid_ch1); //child2 joins the group whose group id is same as child1's pid
      if (err == -1) {
        perror("pgid");
        if (errno == EPERM){
          printf("eperm\n");
      ...
      myargs[1] = strdup("world");   // argument: "firefox"
    ...
    char *myargs[2];
    myargs[0] = strdup("./a");   // program: "top" (writes to stdout which is now pipe)
    // myargs[1] = strdup("sig_ex3.c");
    myargs[1] = NULL;     
    // while(1){}      
